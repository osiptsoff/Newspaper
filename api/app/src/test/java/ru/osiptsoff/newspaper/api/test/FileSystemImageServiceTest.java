...
import ru.osiptsoff.newspaper.api.environment.FileSystemImageServiceTestEnvironment;
...
public class FileSystemImageServiceTest {
    ...
    public void complexTest() throws IOException {
        Image image = env.getImageService().saveImage(
            env.getTestFile(),
            env.getContentType(),
            env.getTestNews().getId()
        );

        Image dbImage = env.getImageService().findImage(env.getTestNews().getId());
